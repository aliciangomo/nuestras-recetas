...
function usePhotoUpload(initial) {
  const [url, setUrl] = useState(initial);        // Cloudinary URL — what gets saved
  const [preview, setPreview] = useState(initial); // may be blob: while uploading
  const [uploading, setUploading] = useState(false);
  const lastUrl = useRef(initial);
  ...
  const onFile = async (e) => {
    ...
    setUploading(true);
    try {
      const cloud = await uploadPhoto(f);
      lastUrl.current = cloud;
      setUrl(cloud);
      setPreview(cloud);
    } catch {
      setPreview(lastUrl.current);
  ...
  return { url, preview, uploading, open, input };
}
...
export function AddRecipe({ onBack, onAdd, onUpdate, initialRecipe, accent, screenBg }) {
  ...
  const { url: photoUrl, preview: photoPreview, uploading: mainUploading, open: openPhoto, input: photoInput } = usePhotoUpload(isEdit ? initialRecipe.photo : null);
  const [extraUploading, setExtraUploading] = useState(0); // count of in-progress extra uploads
  ...
  const addExtraPhoto = async (e) => {
    ...
    e.target.value = '';
    const local = URL.createObjectURL(f);
    setForm(prev => ({ ...prev, photos: [...prev.photos, local] }));
    setExtraUploading(n => n + 1);
    try {
      const cloud = await uploadPhoto(f);
      setForm(prev => ({ ...prev, photos: prev.photos.map(p => p === local ? cloud : p) }));
    } catch {
      setForm(prev => ({ ...prev, photos: prev.photos.filter(p => p !== local) }));
    } finally {
      URL.revokeObjectURL(local);
      setExtraUploading(n => n - 1);
    }
  ...
  const uploadingAny = mainUploading || extraUploading > 0;

  const save = async () => {
    if (!form.title.trim() || saving || uploadingAny) return;
    setSaving(true);
    try {
      const updated = {
  ...
  const valid = form.title.trim().length > 0 && !uploadingAny;
            ...
            <div onClick={openPhoto} style={{ marginTop:14, height:170, borderRadius:10, overflow:'hidden', position:'relative', cursor:'pointer', background:photo ? '#f0f0f0' : 'transparent', border:photo ? 'none' : '1.5px dashed rgba(0,0,0,0.18)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8, color:MUTED }}>
              {mainUploading ? (
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8, color:MUTED }}>
                  ...
                  <div style={{ ...T.body, fontSize:13 }}>Subiendo foto…</div>
                </div>
              ) : photoPreview ? (
                <>
                  <Photo src={photoPreview} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }}/>
