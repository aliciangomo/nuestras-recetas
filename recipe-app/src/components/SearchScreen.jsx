...
export function SearchScreen({ recipes, onRecipe, accent, screenBg, palette }) {
  ...
  const recent = ['lemon', 'sunday', 'pasta', 'banana'];
  const browse = ['Quick (under 30m)', 'Vegetarian', 'Family classics', 'Imported'];
  ...
  return (
    <div className="fade-in" style={{ height:'100%', display:'flex', flexDirection:'column', background:screenBg }}>
      <div style={{ padding:'68px 24px 4px', flexShrink:0 }}>
        <div style={{ ...T.cursive, fontSize:32, lineHeight:1 }}>Search</div>
        <div style={{ ...T.meta, marginTop:2, fontStyle:'italic' }}>find that recipe</div>
      </div>
      ...
      <div style={{ padding:'14px 24px 12px', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, background:palette.accentSoft, borderRadius:10, padding:'10px 14px' }}>
          ...
          <input
            ...
            placeholder="Recipes, ingredients, categories…"
            style={{ flex:1, background:'none', border:'none', outline:'none', ...T.body, fontSize:15 }}
          />
          {q && <button onClick={() => setQ('')} style={{ ...T.meta, fontSize:13, background:'none', border:'none', cursor:'pointer', color:MUTED, padding:0 }}>Clear</button>}
        </div>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto' }}>
        {!q.trim() ? (
          <div style={{ padding:'8px 24px' }}>
            <div style={{ ...T.label, marginBottom:10, color:accent }}>recent searches</div>
            ...
            <div style={{ ...T.label, marginTop:28, marginBottom:14, color:accent }}>browse by</div>
          ...
          <div>
            <div style={{ ...T.label, padding:'4px 24px 8px' }}>{results.length} {results.length === 1 ? 'result' : 'results'} for "{q}"</div>
            {results.length === 0 ? (
              <div style={{ ...T.meta, padding:'24px', textAlign:'center', fontStyle:'italic' }}>No recipes match. Try a different word.</div>
            ) : (
              results.map(r => (
